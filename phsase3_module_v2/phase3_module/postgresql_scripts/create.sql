...
CREATE VIEW view_get_db_table_names AS
SELECT table_name FROM information_schema.tables
WHERE table_type='BASE TABLE' AND table_schema='public';

CREATE VIEW view_get_db_tables_row_num AS
SELECT relname,n_live_tup FROM pg_stat_user_tables
ORDER BY n_live_tup DESC;

CREATE VIEW view_get_num_of_db_tables AS
...
Create procedure usp_insert_student(s_id int,s_name varchar(30), s_surname varchar(30), s_dep_code varchar(10))
    Language sql
As $$ insert into student values((s_id)::INTEGER, s_name, s_surname, s_dep_code)  $$;
